...
(function(){
  ...
  document.addEventListener('click',function(e){var t=e.target&&e.target.closest?e.target:null;if(!t)return;var cryptoOpen=t.closest('[data-mprofile-crypto-open]');if(cryptoOpen){e.preventDefault();e.stopImmediatePropagation();openCrypto();return;}var cryptoClose=t.closest('[data-mprofile-crypto-close]');if(cryptoClose){e.preventDefault();e.stopImmediatePropagation();closeCrypto();return;}var cryptoOption=t.closest('#mprofileCryptoPopup .checkbox-control-content-bc');if(cryptoOption){e.preventDefault();e.stopImmediatePropagation();chooseCrypto(cryptoOption);return;}var closeBtn=t.closest('[data-mprofile-payment-close]');if(closeBtn){e.preventDefault();e.stopImmediatePropagation();window.location.href=balanceUrl('history',{fromPayment:'1'});return;}var submitBtn=t.closest('#mprofilePaymentModal .mprofile-payment-submit');if(submitBtn){e.preventDefault();e.stopImmediatePropagation();submit();return;}var dep=t.closest('[data-mbalance-method]'),wdr=t.closest('[data-mbalance-withdraw-method]');var card=dep||wdr;if(!card)return;e.preventDefault();e.stopImmediatePropagation();var kind=wdr?'withdraw':'deposit';loadMethods().then(function(){open(kind,findMethod(kind,card));});},true);
