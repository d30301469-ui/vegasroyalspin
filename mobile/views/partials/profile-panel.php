...
(function(){
  ...
  function withdrawNetworkId(v){var n=String(v||'').toUpperCase();if(n.indexOf('TRC20')!==-1||n.indexOf('TRC-20')!==-1||n.indexOf('TRON')!==-1||n.indexOf('TRX')!==-1)return'65bd7be5964700005d002ae5';if(n.indexOf('BİTCO')!==-1||n.indexOf('BITCO')!==-1||n.indexOf('BTC')!==-1)return'65bd7bba964700005d002ae1';if(n.indexOf('LTC')!==-1||n.indexOf('LITE')!==-1)return'65bd7bc1964700005d002ae2';return'65bd7bd5964700005d002ae4';}
  ...
  function chooseCrypto(option){if(!option)return;var value=option.getAttribute('data-option-value')||'',label=option.getAttribute('data-option-label')||option.textContent.trim(),input=document.getElementById('mprofilePaymentCryptoType'),network=document.getElementById('mprofilePaymentNetwork'),labelEl=document.getElementById('mprofilePaymentCryptoLabel');if(input)input.value=value;if(network)network.value=withdrawNetworkId(label||value);if(labelEl)labelEl.textContent=label.toLowerCase()==='tron'?'tron':label;document.querySelectorAll('#mprofileCryptoPopup .checkbox-control-content-bc').forEach(function(item){item.classList.toggle('active',item===option);});closeCrypto();}
