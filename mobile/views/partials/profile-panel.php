...
(function(){
  ...
  if(!window.__mprofileCryptoFetchPatched&&window.fetch){window.__mprofileCryptoFetchPatched=true;var nativeFetch=window.fetch.bind(window);window.fetch=function(input,init){try{var url=String(input&&input.url?input.url:input||'');if(url.indexOf('/api/v2/deposit-payment')!==-1&&init&&init.body){var cryptoType=document.getElementById('mprofilePaymentCryptoType'),body=JSON.parse(init.body);if(cryptoType&&cryptoType.value){body.input_fields=body.input_fields||{};body.input_fields.crypto_type=cryptoType.value;init=Object.assign({},init,{body:JSON.stringify(body)});}}}catch(err){}return nativeFetch(input,init);};}
