...
(function(){
  ...
  function depositCryptoOptions(){return [['FTN','FTN ERC-20'],['BTC','BTC'],['USDT_T','USDT(TRC20)'],['USDT','USDT(ERC20)'],['USDC','USDC(ERC20)'],['TRX','TRON'],['ETH','ETH'],['USDB','BSC-USD'],['BNBB','BSC']];}
  ...
  function cryptoOptions(){return active&&active.kind==='withdraw'?withdrawCryptoOptions():depositCryptoOptions();}
  ...
  if(!window.__mprofileCryptoFetchPatched&&window.fetch){window.__mprofileCryptoFetchPatched=true;var nativeFetch=window.fetch.bind(window);window.fetch=function(input,init){try{var url=String(input&&input.url?input.url:input||'');if(init&&init.body&&(url.indexOf('/api/v2/deposit-payment')!==-1||url.indexOf('/api/v2/withdraw-payment')!==-1)){var body=JSON.parse(init.body);if(url.indexOf('/api/v2/deposit-payment')!==-1){var cryptoType=document.getElementById('mprofilePaymentCryptoType');if(cryptoType&&cryptoType.value){body.input_fields=body.input_fields||{};body.input_fields.crypto_type=cryptoType.value;}}if(url.indexOf('/api/v2/withdraw-payment')!==-1){var network=document.getElementById('mprofilePaymentNetwork');if(network&&network.value){body.input_fields=body.input_fields||{};body.input_fields.bank_id=network.value;delete body.input_fields.crypto_network;}}init=Object.assign({},init,{body:JSON.stringify(body)});}}catch(err){}return nativeFetch(input,init);};}
