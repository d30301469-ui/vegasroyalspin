...
<div class="drakon-grid">
    <form id="drakonSettingsForm" class="drakon-card" method="post" action="<?= $text(AdminAuth::url('/drakon/settings')) ?>">
        ...
        <div class="field">
            ...
            <input id="site_endpoint" class="input" type="url" name="site_endpoint" value="<?= $text($configRow['site_endpoint'] ?? '') ?>" placeholder="<?= $text($backendBase) ?>">
            <p class="text-muted" style="margin:.35rem 0 0;font-size:12px">Drakon agent paneline yazılacak site adresi. Örnek: <code><?= $text($backendBase) ?></code></p>
