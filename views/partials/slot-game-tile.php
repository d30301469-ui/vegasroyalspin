...
$runtimeCardActivateJs = 'if(window.__slotGameCardActivate){window.__slotGameCardActivate(event,' . $playHrefJson . ');}else{' . $runtimePlayIntentJs . '}';
$demoHrefJson = (string) json_encode($demoHref, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_SLASHES);
$runtimeDemoIntentJs = 'if(event){event.preventDefault();event.stopPropagation();}if(window.__slotHandleDemoIntent){window.__slotHandleDemoIntent(event,' . $demoHrefJson . ');}else{window.location.href=' . $demoHrefJson . ';}';
...
<div class="<?= htmlspecialchars($contentClass, ENT_QUOTES, 'UTF-8') ?>"
     data-favorite-kind="<?= htmlspecialchars((string) ($slotFavoriteKind ?? 'slot'), ENT_QUOTES, 'UTF-8') ?>"
