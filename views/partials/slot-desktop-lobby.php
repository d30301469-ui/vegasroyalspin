<?php
/**
 * Desktop Lobby — casinomilyon622 casinoLobbyContainer markup.
 */
...
$lobbyLiveGames = isset($lobbyLiveGames) && is_array($lobbyLiveGames) ? $lobbyLiveGames : [];
...
$lobbyLiveTotal = (int) ($lobbyLiveTotal ?? count($lobbyLiveGames));
...
$isLiveLobby = ((int) ($slotGameType ?? 0) === 1) || !empty($slotPageIsLive);
...
$lobbyPrimaryTitle = (string) ($lobbySectionTitles['primary'] ?? ($isLiveLobby ? 'CANLI CASİNO' : 'CASİNO OYUNLARI'));
$lobbyPopularTitle = (string) ($lobbySectionTitles['popular'] ?? ($isLiveLobby ? 'Popüler Oyunlar' : 'Casino Games'));
$lobbyHighWinsTitle = (string) ($lobbySectionTitles['highWins'] ?? ($isLiveLobby ? 'Rulet' : 'En Yüksek Kazançlar'));
$lobbyTournamentsTitle = (string) ($lobbySectionTitles['tournaments'] ?? ($isLiveLobby ? 'Blackjack' : 'Turnuvalar'));
$lobbyLiveTitle = (string) ($lobbySectionTitles['live'] ?? 'CANLI CASİNO');
...
$lobbyLiveHref = (string) ($lobbySectionHrefs['live'] ?? ($isLiveLobby ? ($slotPageBaseUrl . '?sort=game-show') : '/livecasino'));
...
<div class="casinoLobbyContainer" id="casinoLobbySections">
    ...
    <?php if (!$isLiveLobby || $lobbyLiveGames !== []): ?>
    <div class="hm-row-bc" style="grid-template-columns: 12fr;">
        <div class="hm-row-bc-inner">
            <div class="casinoHorizontalGamesList casinoGamesWidget">
                <?php $renderSectionTitle($lobbyLiveTitle, $lobbyLiveTotal > 0 ? '+' . $lobbyLiveTotal : '', $lobbyLiveHref, 'lobby-live'); ?>
                <?php if ($lobbyLiveGames !== []): ?>
                <div class="carouselWrapper carousel" data-lobby-carousel-el="lobby-live">
                    <div class="swiper lobby-games-swiper">
                        <div class="swiper-wrapper">
                            <?php
                            $prevFavoriteKind = $slotFavoriteKind ?? 'slot';
                            $slotFavoriteKind = 'live';
                            foreach ($lobbyLiveGames as $game):
                            ?>
                            <div class="swiper-slide">
                                <?php include VIEW_PATH . '/partials/slot-game-tile.php'; ?>
                            </div>
                            <?php
                            endforeach;
                            $slotFavoriteKind = $prevFavoriteKind;
                            ?>
                        </div>
                    </div>
                </div>
                <?php else: ?>
                <div class="carouselWrapper carousel carouselArrowsDisabled lobby-skeleton-carousel" data-lobby-carousel-el="lobby-live">
                    <div class="swiper">
                        <div class="swiper-wrapper">
                            <?php for ($i = 0; $i < 8; $i++): ?>
                            <div class="swiper-slide">
