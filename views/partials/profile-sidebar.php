...
$sidebar_display_name = null;
if (!empty($user_info['first_name']) || !empty($user_info['surname'])) {
    $sidebar_display_name = trim(($user_info['first_name'] ?? '') . ' ' . ($user_info['surname'] ?? ''));
}
if (empty($sidebar_display_name) && !empty($firstName) && isset($surname)) {
    $sidebar_display_name = trim(($firstName ?? '') . ' ' . ($surname ?? ''));
}
if (empty($sidebar_display_name)) {
