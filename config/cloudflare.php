...
if (!function_exists('cloudflare_ssl_enabled')) {
    function cloudflare_ssl_enabled(): bool
    {
...
if (!function_exists('origin_http_only')) {
    function origin_http_only(): bool
    {
        if (!function_exists('frontend_env_bool')) {
            ...
            return cloudflare_ssl_enabled();
        }

        return frontend_env_bool('ORIGIN_HTTP') || cloudflare_ssl_enabled();
    }
}
...
if (!function_exists('request_is_https')) {
    ...
    function request_is_https(): bool
    {
        ...
        if (cloudflare_ssl_enabled() && !empty($_SERVER['HTTP_CF_RAY'])) {
            return true;
        }
...
if (!function_exists('public_url_scheme')) {
     ...
     */
    function public_url_scheme(string $fallback = 'https'): string
    {
        if (cloudflare_ssl_enabled()) {
            return 'https';
        }

        return request_is_https() ? 'https' : $fallback;
    }
}

if (!function_exists('is_local_dev_host')) {
    function is_local_dev_host(string $host): bool
    {
...
if (!function_exists('coerce_public_https_url')) {
     ...
     */
    function coerce_public_https_url(string $url): string
    {
        $url = trim($url);
        if ($url === '' || !cloudflare_ssl_enabled()) {
            return $url;
        }
...
if (!function_exists('build_public_origin_url')) {
    function build_public_origin_url(string $httpHost, string $fallbackScheme = 'https'): string
    {
        $httpHost = preg_replace('/:\d+$/', '', trim($httpHost)) ?: 'localhost';
        $scheme = public_url_scheme($fallbackScheme);
