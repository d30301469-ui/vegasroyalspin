...
final class DrakonService
{
    ...
    public static function verifyWebhookRequest(PDO $pdo, string $rawBody, array $server, bool $healthProbe = false): array
    {
        ...
        // Webhook Drakon panel doğrulaması için açık kalmalı; oyun launch activeConfig() ile kontrol edilir.
    ...
    public static function launch(PDO $pdo, ?array $user, array $input): array
    {
        ...
        if (empty($publicProbe['ok'])) {
            ...
            // HTTP 0 = ağ timeout/erişilemez. Drakon webhook'u oyun sırasında dışarıdan çağırır;
            // sunucunun kendi ngrok/tünel URL'ine self-referential probe yapması güvenilmez.
            // Non-production'da timeout hatası launch'ı engellememeli.
    ...
    private static function executeAuthRequest(string $url, string $auth): array
    {
        ...
        for ($attempt = 0; $attempt < 6; $attempt++) {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_CONNECTTIMEOUT => 5,
                CURLOPT_TIMEOUT        => 15,
                CURLOPT_FOLLOWLOCATION => false, // 3xx yönlendirmeleri elle takip ediyoruz
            ...
            // 3xx: yeni URL'e aynı Authorization balkıyla el ile yönlendir
    ...
    public static function webhookPublicUrl(string $siteEndpoint = ''): string
    {
        ...
        $base = rtrim(trim($siteEndpoint !== '' ? $siteEndpoint : self::canonicalBackendSiteUrl()), '/');
        if ($base === '') {
            return '/drakon_api';
        }
