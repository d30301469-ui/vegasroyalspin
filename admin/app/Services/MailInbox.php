...
if (!function_exists('mail_imap_available')) {
    function mail_imap_available(): bool
    {
        return function_exists('imap_open') && extension_loaded('imap');
    }
}

if (!function_exists('mail_imap_diagnostics')) {
    function mail_imap_diagnostics(): string
    {
        $lines = [];
...
if (!function_exists('mail_imap_apply_timeouts')) {
    /**
     ...
     */
    function mail_imap_apply_timeouts(int $seconds = 6): void
    {
...
if (!function_exists('mail_imap_port_open')) {
    /** IMAP portu erisilebilir mi? imap_open'dan once hizli TCP kontrolu. */
    function mail_imap_port_open(string $host, int $port, float $timeout = 4.0): bool
    {
        $errno = 0;
        $errstr = '';
...
if (!function_exists('mail_fetch_inbox')) {
    /**
     * @param array<string,mixed> $settings mail_settings satırı
     * @return array{ok:bool,error:string,messages:list<array<string,mixed>>}
     */
    function mail_fetch_inbox(array $settings, int $limit = 40, float $previewBudget = 6.0): array
    {
        $conn = mail_imap_connect($settings);
        ...
        try {
            ...
            foreach ($overview as $item) {
                ...
                $subject = mail_imap_decode_mime((string) ($item->subject ?? '(konu yok)'));
                $from = mail_imap_decode_mime((string) ($item->from ?? ''));
                ...
                if ($msgNo > 0 && microtime(true) < $previewDeadline) {
                    $parts = mail_imap_extract_bodies($inbox, $msgNo);
...
if (!function_exists('mail_imap_connect')) {
    /**
     * @param array<string,mixed> $settings
     * @return array{ok:bool,error:string,inbox?:resource|\IMAP\Connection,user?:string}
     */
    function mail_imap_connect(array $settings): array
    {
        if (!mail_imap_available()) {
            $disabled = (string) ini_get('disable_functions');
            ...
            if ($imapDisabled) {
                return [
                    ...
                    'error' => 'IMAP eklentisi yüklü ama imap_open disable_functions ile engellenmiş. '
                        ...
                        . 'Tanı: ' . mail_imap_diagnostics(),
                ];
            }
            return [
                'ok' => false,
                'error' => 'PHP imap eklentisi yok. Tanı: ' . mail_imap_diagnostics(),
            ];
        }
        if (isset($settings['imap_enabled']) && (int) $settings['imap_enabled'] === 0) {
        ...
        mail_imap_apply_timeouts(6);
        if (!mail_imap_port_open($host, $port, 4.0)) {
            return [
                'ok' => false,
...
if (!function_exists('mail_fetch_message')) {
    /**
     * @param array<string,mixed> $settings
     * @return array{ok:bool,error:string,message?:array<string,mixed>}
     */
    function mail_fetch_message(array $settings, int $uid): array
    {
        if ($uid <= 0) {
        ...
        try {
            ...
            return [
                ...
                'message' => [
                    ...
                    'msgno' => $msgNo,
                    'from' => mail_imap_decode_mime((string) ($item->from ?? '')),
                    'to' => mail_imap_decode_mime((string) ($item->to ?? '')),
                    'subject' => mail_imap_decode_mime((string) ($item->subject ?? '(konu yok)')),
                    'date' => (string) ($item->date ?? ''),
                    'text' => $parts['text'],
                    'html' => $parts['html'],
...
if (!function_exists('mail_imap_decode_part')) {
    function mail_imap_decode_part(string $body, int $encoding): string
    {
        if ($encoding === 3) {
...
if (!function_exists('mail_imap_extract_bodies')) {
    /**
     * @param resource|\IMAP\Connection $inbox
     * @return array{text:string,html:string}
     */
    function mail_imap_extract_bodies($inbox, int $msgNo): array
    {
        $text = '';
        $html = '';
        ...
        $walk = static function ($struct, string $prefix) use (&$walk, &$text, &$html, $inbox, $msgNo): void {
            ...
            $body = mail_imap_decode_part($body, $encoding);
        ...
        if ($text === '' && $html === '') {
            ...
            $text = trim(mail_imap_decode_part($raw, $encoding));
        }
...
if (!function_exists('mail_imap_decode_mime')) {
    function mail_imap_decode_mime(string $value): string
    {
        $value = trim($value);
        if ($value === '') {
            return '';
