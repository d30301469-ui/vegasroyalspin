...
$base = AdminAuth::url('/compliance/aml-alerts');
...
<section class="hero">
    <div class="hero-text">
        ...
        <p class="hero-sub">Yatırım/çekim, structuring, kimlik paylaşımı ve dönemsel oran sinyallerini inceleyin.</p>
    </div>
...
<div class="cmp-charts">
    <div class="cmp-chart-card">
        ...
        <div class="cmp-chart-wrap"><canvas id="aml-severity"></canvas></div>
    </div>
    <div class="cmp-chart-card">
        <div class="cmp-chart-head"><h3 class="cmp-chart-title">Durum dağılımı</h3></div>
        <div class="cmp-chart-wrap"><canvas id="aml-status"></canvas></div>
    </div>
    <div class="cmp-chart-card wide">
        <div class="cmp-chart-head"><h3 class="cmp-chart-title">Açık kurallar</h3></div>
        <div class="cmp-chart-wrap"><canvas id="aml-rules"></canvas></div>
    </div>
    <div class="cmp-chart-card full">
        <div class="cmp-chart-head"><h3 class="cmp-chart-title">14 günlük trend</h3></div>
        <div class="cmp-chart-wrap tall"><canvas id="aml-trend"></canvas></div>
    </div>
</div>
<?php
$chartPrefix = 'aml';
    ...
    <form method="get" action="<?= $text($base) ?>" style="display:flex;flex-wrap:wrap;gap:8px;align-items:end">
        ...
        <label>Kural
            <select class="input" name="rule">
                ...
                <?php
                $amlRules = [
                    'large_withdraw', 'large_deposit', 'rapid_deposit_withdraw', 'new_account_large_withdraw',
                    'withdraw_without_deposit', 'withdraw_exceeds_deposits', 'withdraw_without_kyc',
                    'deposit_structuring', 'shared_identity', 'period_withdraw_ratio',
                ];
                foreach ($amlRules as $code): ?>
...
<section class="card">
    ...
    $ignoreUrl = AdminAuth::url('/compliance/aml/ignore');
