...
(function() {
    ...
    const config = window.BGAMING_CONFIG || { currentPage: 1, nextPage: 2, pageSize: 30, loggedIn: false, search: '', providers: [], sort: '', totalSlots: 0, remainingGames: 0, showLoadMore: false };
    ...
    const API_EXTRA_PARAMS = Object.assign({ source: 'bgaming' }, (config.apiParams && typeof config.apiParams === 'object') ? config.apiParams : {});
    ...
    const catArrowLeft       = document.getElementById('bgamingCatArrowLeft')
        || document.getElementById('catArrowLeft');
    const catArrowRight      = document.getElementById('bgamingCatArrowRight')
        || document.getElementById('catArrowRight');
    const catArrowShadowLeft = document.getElementById('bgamingCatArrowShadowLeft');
    const catArrowShadowRight = document.getElementById('bgamingCatArrowShadowRight');
    // CM622 HorizontalScroll: overflow:hidden viewport + translateX on inner track.
    const catScrollInner     = document.getElementById('bgamingCategoryRailInner')
        || document.querySelector('.casinoNavigationAndFilters .casinoCategories .horizontal-scroll__inner');
    const catScrollViewport  = document.getElementById('bgamingCategoryRail')
    ...
    window.__bgamingHandleDemoIntent = handleDemoIntent;
    ...
    function activateGameCard(event, url) {
        ...
        if (document.body.classList.contains('mobile-site') && ACTION_BUTTONS) {
            if (event) {
                event.preventDefault();
                event.stopPropagation();
            }
            var card = event && event.currentTarget
                ? event.currentTarget
                : (event && event.target ? event.target.closest('.casinoGameItemContent') : null);
            if (!card) {
                handlePlayIntent(event, url);
                return;
            }
            var isActive = card.classList.contains('overlay-active');
            document.querySelectorAll('.casinoGameItemContent.overlay-active, .game-item.overlay-active, .game-cta.overlay-active')
                .forEach(function (el) { el.classList.remove('overlay-active'); });
            if (!isActive) {
    ...
    window.__bgamingGameCardActivate = activateGameCard;
    ...
    function demoPlayClickJs(demoUrlJs) {
        return "if(event){event.preventDefault();event.stopPropagation();}window.__bgamingHandleDemoIntent&&window.__bgamingHandleDemoIntent(event,'" + demoUrlJs + "')";
    }
    ...
    function renderGameItem(game) {
        ...
        const blockHtml =
            ...
            '</div>';
        return (
            '<div class="casinoGameItemContent casinoGameItemContent--regular" data-favorite-kind="' + escapeHtml(FAVORITE_KIND) + '" data-game-id="' + gameIdEsc + '"' + catalogAttr + ' onclick="window.__bgamingGameCardActivate&&window.__bgamingGameCardActivate(event,\'' + gameUrlJs + '\')">' +
    ...
    function normalizeApiResponse(data) {
        ...
        var games = rawGames.map(function(game) {
            ...
            return mapped;
        }).filter(function(game) { return !!game; });
    ...
    function buildCategoryHref(sort) {
        var basePath = window.location.pathname || '/bgaming';
        ...
        catScroll.querySelectorAll('.ds-chip').forEach(function(chip) {
            ...
            if (text === 'tüm oyunlar') {
                ...
                chip.classList.toggle(
                    ...
                    !LOBBY_MODE
                    && state.sort === ''
                    && (config.isLive === true || String(config.view || '') === 'all')
                );
                return;
            }
            var sort = getMobileOriginalCategorySort(chip);
    ...
    function isDesktopLobbyAutoScroll() {
        return !!(window.matchMedia && window.matchMedia('(min-width: 1025px)').matches)
            && !prefersReducedMotion();
    }

    function bindLobbyAutoplay(swiper, swiperEl) {
        if (!swiper || !swiper.autoplay || !swiperEl) return;
        var pause = function() {
            try { swiper.autoplay.stop(); } catch (e) {}
            swiperEl.classList.add('is-autoplay-paused');
        };
        var resume = function() {
            if (!isDesktopLobbyAutoScroll()) return;
            try { swiper.autoplay.start(); } catch (e) {}
            swiperEl.classList.remove('is-autoplay-paused');
        };
        swiperEl.addEventListener('mouseenter', pause);
        swiperEl.addEventListener('mouseleave', resume);
        swiperEl.addEventListener('focusin', pause);
        swiperEl.addEventListener('focusout', function(ev) {
            if (!swiperEl.contains(ev.relatedTarget)) resume();
        });
        // Bounce at ends when loop is disabled (short catalogues).
    ...
    function initLobbyCarousels() {
        ...
        var enableAuto = isDesktopLobbyAutoScroll();
        roots.forEach(function(wrapper, index) {
            ...
            var isGamesRow = swiperEl.classList.contains('lobby-games-swiper');
            var slideCount = swiperEl.querySelectorAll('.swiper-slide').length;
            var useAutoplay = enableAuto && isGamesRow && slideCount > 3;
            var useLoop = useAutoplay && slideCount >= 8;
            var swiper = new Swiper(swiperEl, {
                slidesPerView: 'auto',
                spaceBetween: 16,
                watchOverflow: true,
                resistanceRatio: 0.65,
                speed: useAutoplay ? 720 : 300,
                loop: useLoop,
                loopedSlides: useLoop ? Math.min(slideCount, 12) : undefined,
                autoplay: useAutoplay ? {
                    delay: 2600 + (index % 3) * 350,
            ...
            if (useAutoplay) {
                bindLobbyAutoplay(swiper, swiperEl);
            }
            if (prevBtn) {
