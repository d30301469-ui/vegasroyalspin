 ...
 */
function request_is_fast_public_api(string $path): bool
{
    $path = '/' . trim($path, '/');
...
function public_api_route_from_path(string $path): string
{
...
function handle_public_api_request(?string $requestUri = null): void
{
    $requestUri ??= (string) ($_SERVER['REQUEST_URI'] ?? '/');
    $path = '/' . trim((string) (parse_url($requestUri, PHP_URL_PATH) ?: ''), '/');
    if (!request_is_fast_public_api($path)) {
    ...
    dispatch_frontend_public_api($requestUri);
    exit;
}

function dispatch_frontend_public_api(string $requestUri): void
{
    ...
    $route = public_api_route_from_path($requestUri);
    if ($route === '') {
        if (!headers_sent()) {
            http_response_code(404);
            header('Content-Type: application/json; charset=UTF-8');
