...
require_once shared_package_root() . '/config/backend_api.php';
...
final class BackendApiClient
{
    ...
    private static function detectCachedInternalBaseUrl(): string
    {
        ...
        if ($backendHost === '') {
            $public = self::effectiveMainBaseUrl();
            $backendHost = strtolower((string) (parse_url($public, PHP_URL_HOST) ?: self::defaultBackendHost()));
        }
    ...
    public static function effectiveOutboundHostHeader(): string
    {
        ...
        $public = self::effectiveMainBaseUrl();
        $host = strtolower((string) (parse_url($public, PHP_URL_HOST) ?: self::defaultBackendHost()));
        if (str_starts_with($host, 'api.')) {
            $host = substr($host, 4);
        }

        return $host !== '' ? $host : self::defaultBackendHost();
    }

    private static function defaultBackendHost(): string
    {
        if (!function_exists('deploy_domain') && is_file(shared_package_root() . '/config/deploy_domains.php')) {
            require_once shared_package_root() . '/config/deploy_domains.php';
        }
        if (function_exists('deploy_domain')) {
            $host = strtolower((string) (parse_url(deploy_domain('backend_url'), PHP_URL_HOST) ?: ''));
            if ($host !== '') {
                return $host;
            }
        }
    ...
    private static function resolvePrimaryCainfo(): ?string
    {
        ...
        $caBundle = shared_package_root() . '/config/cacert.pem';
