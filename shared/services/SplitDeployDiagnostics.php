...
final class SplitDeployDiagnostics
{
    ...
    public static function runFrontend(string $root, bool $includeApiProbes = true): array
    {
        ...
        if (!$hasEnv) {
            $result['ok'] = false;
            $result['hints'][] = 'https://vegasroyalspin.com/install — veya: cp ENV.example .env && php scripts/bootstrap-frontend-env.php';
        }
        ...
        if ($hasEnv) {
            ...
            if ($missing !== []) {
                ...
                $result['hints'][] = 'SSH: php scripts/bootstrap-frontend-env.php && MEMBER_JWT_SECRET backend .env ile ayni olmali';
            ...
            if (
                ...
                $result['hints'][] = 'MEMBER_JWT_SECRET hala CHANGE-ME — admin.vegasroyalspin.com .env degerini kopyalayin';
            }
        }
        ...
        if (function_exists('frontend_direct_member_api')) {
            $result['checks']['frontend_direct_member_api'] = frontend_direct_member_api() ? 'yes' : 'no';
        }
        ...
        if ($includeApiProbes && $backendBase !== '' && is_readable($root . '/services/BackendConnectivityProbe.php')) {
            ...
            $backendHost = strtolower((string) (parse_url($backendUrl !== '' ? $backendUrl : $backendBase, PHP_URL_HOST) ?: 'admin.vegasroyalspin.com'));
            ...
            if ($usable) {
                ...
                if (function_exists('should_skip_remote_backend')) {
                    $result['checks']['cms_skip_remote_backend'] = should_skip_remote_backend() ? 'yes' : 'no';
                }
                if (function_exists('cms_api_circuit_is_open')) {
                    $result['checks']['cms_api_circuit_open'] = cms_api_circuit_is_open() ? 'yes' : 'no';
                }
            }
        }
