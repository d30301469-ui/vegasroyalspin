...
final class WageringService
{
    ...
    private static function applyBonusDelta(PDO $pdo, int $userId, float $delta): void
    {
        try {
            ...
            foreach ($rows as $row) {
                ...
                // Çevrim bitmeden bonus bakiyesi tükenirse bonus iptal (cancelled) olur.
                // Aksi halde canlı bonus_balance ile senkronize edilir.
                if ($isComplete) {
                    $pdo->prepare(
    ...
    private static function loyaltyEarn(PDO $pdo, int $userId, float $amount, ?string $walletSource): void
    {
        try {
            if (!class_exists('LoyaltyService', false)) {
                $path = __DIR__ . '/LoyaltyService.php';
                if (is_file($path)) {
                    require_once $path;
                }
            }
            if (class_exists('LoyaltyService', false)) {
                LoyaltyService::earnFromWager($pdo, $userId, $amount, $walletSource, 'wager');
            }
        } catch (Throwable $e) {
            error_log('[WageringService] loyaltyEarn failed: ' . $e->getMessage());
        }
    }

    private static function loyaltyReverse(PDO $pdo, int $userId, float $amount, ?string $walletSource): void
    {
        try {
            if (!class_exists('LoyaltyService', false)) {
                $path = __DIR__ . '/LoyaltyService.php';
            ...
            if (class_exists('LoyaltyService', false)) {
                LoyaltyService::reverseFromWager($pdo, $userId, $amount, $walletSource, 'wager_reverse');
            }
        } catch (Throwable $e) {
            error_log('[WageringService] loyaltyReverse failed: ' . $e->getMessage());
        }
    }
}
