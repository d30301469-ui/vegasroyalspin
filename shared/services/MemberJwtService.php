...
final class MemberJwtService
{
    ...
    public static function secret(): string
    {
        ...
        return hash('sha256', $seed . '|member-jwt-v2');
    }

    /** @return list<string> */
    private static function secretCandidates(): array
    {
        $candidates = [];
        try {
            $candidates[] = self::secret();
        } catch (Throwable) {
        }

        // Non-prod derived secrets previously used a brand pepper; accept both.
        $seed = self::env('APP_KEY') ?: self::env('APP_SECRET');
        if ($seed === '' && defined('SITE_URL')) {
            $seed = (string) SITE_URL;
        }
        ...
        if ($seed !== '') {
            $candidates[] = hash('sha256', $seed . '|member-jwt-v2');
            $candidates[] = hash('sha256', $seed . '|maltabet-v2-member-jwt');
        }
...
SQL;
    ...
    /**
     * Kullanıcının tüm aktif JWT kayıtlarını iptal eder (ban / zorla çıkış).
     */
    public static function revokeAllForUser(PDO $pdo, int $userId): int
    {
        if ($userId <= 0) {
            return 0;
        }
        ...
        try {
            ...
            error_log('[MemberJwtService] revokeAllForUser failed: ' . $e->getMessage());
    ...
    private static function hasValidSignature(string $jwt): bool
    {
        ...
        foreach (self::secretCandidates() as $secret) {
