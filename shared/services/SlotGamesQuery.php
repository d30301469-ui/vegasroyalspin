...
if (!defined('ADMIN_APP_PATH')) {
    define('ADMIN_APP_PATH', shared_project_root() . '/admin/app');
}
...
final class SlotGamesQuery
{
    ...
    public static function gamesPage(int $gameType, string $searchTerm, array $providers, int $limit, int $page, string $sort = '', array $extraQuery = []): array
    {
        ...
        // BGaming is a direct SoftSwiss integration with its own page/query.
        $sourceHint = strtolower(trim((string) ($extraQuery['source'] ?? '')));
        if ($sourceHint === 'bgaming') {
            $bgamingPath = __DIR__ . '/BgamingGamesQuery.php';
            if (is_file($bgamingPath)) {
                require_once $bgamingPath;
            }
            if (class_exists('BgamingGamesQuery', false)) {
                return BgamingGamesQuery::page(
                    $searchTerm,
                    $limit,
                    $page,
                    strtolower(trim($sort)) === 'all' ? '' : $sort
                );
            }
        }
        ...
        // Popular / liked: prefer featured rows, but do not hard-require them
        // (empty featured catalogs would blank the filtered page).
        if ($sort === 'popular' || $sort === 'liked') {
            $query['prefer_featured'] = '1';
        }
    ...
    /**
     * Local slot/live catalogue union (Casino Aggregator + optional GSC fallback).
     * BGaming is owned by BgamingGamesQuery and is not included here.
     *
    ...
    private static function combinedCatalogPage(PDO $pdo, array $query, int $limit, int $page): array
    {
        ...
        $preferFeatured = (string) ($query['prefer_featured'] ?? '') === '1';
        $sortKey = strtolower(trim((string) ($query['sort'] ?? '')));
        // Slot lobby default = Casino Aggregator.
        // BGaming is owned by BgamingGamesQuery; live casino by LiveCasinoQuery (GSC+).
        $source       = strtolower(trim((string) ($query['source'] ?? '')));

        $union = [];
        // Live lobby default = all Casino Aggregator live vendors (game_type=2).
        // Explicit source=gsc keeps the legacy GSC+ live catalog.
        if ($gameType === 1 && in_array($source, ['', 'aggregator', 'casino_aggregator'], true)) {
            $liveMatch = class_exists('CasinoAggregatorService', false)
                ? CasinoAggregatorService::liveVendorSqlMatch('g.vendor_code')
                : "LOWER(g.vendor_code) LIKE 'casino-%'";
            $slotOnly = class_exists('CasinoAggregatorService', false)
                ? CasinoAggregatorService::slotOnlyVendorSqlMatch('g.vendor_code')
                : "LOWER(g.vendor_code) LIKE 'slot-%'";
            $union[] = "SELECT
                ...
                WHERE g.is_active = 1 AND v.is_active = 1
                  ...
                  AND NOT ({$slotOnly})";
        } elseif ($gameType === 1 && ($source === 'gsc') && self::tableHasColumn($pdo, 'gsc_games', 'game_code')) {
        ...
        // Category chips → practical catalog filters for aggregator slots.
        $nameHay = "LOWER(CONCAT(COALESCE(name,''),' ',COALESCE(provider,''),' ',COALESCE(provider_code,''),' ',COALESCE(raw_payload,'')))";
        if ($sortKey === 'jackpots') {
            $where[] = "({$nameHay} LIKE '%jackpot%' OR {$nameHay} LIKE '% Progressive %' OR {$nameHay} LIKE '%progressive%')";
        } elseif ($sortKey === 'bonus-buy' || $sortKey === 'freespin') {
            $where[] = "({$nameHay} LIKE '%bonus buy%' OR {$nameHay} LIKE '%buy bonus%' OR {$nameHay} LIKE '%buy feature%' OR {$nameHay} LIKE '%buybonus%' OR {$nameHay} LIKE '%freespin%' OR {$nameHay} LIKE '%free spin%' OR {$nameHay} LIKE '%\"buybonus\":true%' OR {$nameHay} LIKE '%\"hasbuybonus\":true%')";
        } elseif ($sortKey === 'crash') {
            $where[] = "({$nameHay} LIKE '%crash%' OR {$nameHay} LIKE '%aviator%' OR {$nameHay} LIKE '%jetx%' OR {$nameHay} LIKE '%spribe%' OR {$nameHay} LIKE '%airplane%' OR {$nameHay} LIKE '%rocket%' OR {$nameHay} LIKE '%mines%')";
        } elseif ($sortKey === 'table') {
            $where[] = "({$nameHay} LIKE '%roulette%' OR {$nameHay} LIKE '%blackjack%' OR {$nameHay} LIKE '%baccarat%' OR {$nameHay} LIKE '%poker%' OR {$nameHay} LIKE '%sic bo%' OR {$nameHay} LIKE '%andar%')";
        } elseif ($sortKey === 'instant') {
            $where[] = "({$nameHay} LIKE '%instant%' OR {$nameHay} LIKE '%scratch%' OR {$nameHay} LIKE '%keno%' OR {$nameHay} LIKE '%bingo%' OR {$nameHay} LIKE '%plinko%')";
        } elseif ($sortKey === 'video' || $sortKey === 'slots') {
            $where[] = "NOT ({$nameHay} LIKE '%crash%' OR {$nameHay} LIKE '%aviator%' OR {$nameHay} LIKE '%jetx%' OR {$nameHay} LIKE '%roulette%' OR {$nameHay} LIKE '%blackjack%')";
        } elseif ($sortKey === 'new') {
            // No hard filter — newest rows via ORDER BY below.
        } elseif ($gameType === 1) {
            // Live-casino category chips (roulette / blackjack / …).
            $liveQueryPath = __DIR__ . '/LiveCasinoQuery.php';
            if (!class_exists('LiveCasinoQuery', false) && is_file($liveQueryPath)) {
                require_once $liveQueryPath;
            }
            if (class_exists('LiveCasinoQuery', false) && method_exists('LiveCasinoQuery', 'liveCategorySqlMatch')) {
                $catMatch = LiveCasinoQuery::liveCategorySqlMatch($sortKey, 'name');
                if ($catMatch !== '') {
                    $where[] = $catMatch;
                }
            }
    ...
    private static function localProviders(int $gameType): array
    {
        ...
        try {
            ...
            // Live casino providers = all Casino Aggregator live vendors.
            if ($gameType === 1) {
                $liveMatch = class_exists('CasinoAggregatorService', false)
                    ? CasinoAggregatorService::liveVendorSqlMatch('g.vendor_code')
                    : "LOWER(g.vendor_code) LIKE 'casino-%'";
