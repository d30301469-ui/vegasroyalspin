...
final class BackendMemberApiProxy
{
    ...
    public static function forward(string $route): void
    {
        ...
        if (
            !$criticalRoute
            && function_exists('member_api_circuit_is_open')
            && member_api_circuit_is_open()
        ) {
            ...
            self::jsonError(503, 'Backend API geçici olarak kullanılamıyor (circuit açık). Birkaç saniye sonra tekrar deneyin.', [
                'circuit' => 'open',
                'retry_after_seconds' => function_exists('member_api_circuit_seconds')
                    ? member_api_circuit_seconds()
                    : 5,
                'hint' => 'Frontend .env: API_BACKEND_INTERNAL_BASE_URL=http://127.0.0.1/api/v2 ve API_BACKEND_INTERNAL_HOST=admin.vegasroyalspin.com',
            ]);
        }
        ...
        if ($base === '') {
            self::jsonError(503, 'Backend API base URL is not configured.', [
                'hint' => 'Frontend .env: API_BACKEND_MAIN_BASE_URL=https://admin.vegasroyalspin.com/api/v2',
                'check' => '/health.php',
            ]);
        }
        ...
        self::jsonError(502, $lastError, [
            ...
            'hint' => '502: backend erişilemiyor — .env: API_BACKEND_MAIN_BASE_URL=https://admin.vegasroyalspin.com/api/v2',
        ]);
    }
     ...
     * Ayrıca gerçek ziyaretçi IP'sini client_ip olarak ekler (affiliate IP atfı için).
     *
    ...
    private static function withReferralCode(string $routeNorm, string $method, ?string $rawBody, ?string $contentType): array
    {
        ...
        if (trim((string) ($decoded['client_ip'] ?? '')) === '') {
            $ip = '';
            foreach (self::clientIpForwardHeaders() as $headerLine) {
                if (stripos($headerLine, 'CF-Connecting-IP:') === 0) {
                    $ip = trim(substr($headerLine, strlen('CF-Connecting-IP:')));
                    break;
                }
            }
            if ($ip !== '') {
                $decoded['client_ip'] = $ip;
            }
        }
    ...
    private static function buildFrontendProxyTrustHeaders(): array
    {
        ...
        $secret = function_exists('frontend_trust_secret')
        ...
        if ($secret === '' || strlen($secret) < 32 || str_contains(strtolower($secret), 'change-me') || str_contains(strtolower($secret), 'changeme') || str_contains(strtolower($secret), 'placeholder')) {
            return [];
        }
    ...
    private static function buildProxyOutboundHeaders(string $routeNorm, ?string $authorization): array
    {
        ...
        if ($authorization !== null && $authorization !== '') {
            if (preg_match('/^\s*Bearer\s+(.+)\s*$/i', $authorization, $matches) === 1) {
                ...
                if ($token !== '') {
                    $headers[] = 'X-App-Member-Jwt: ' . $token;
                }
            }
        }
    ...
    private static function maybeApplyAuthSession(string $route, string $method, array $result): void
    {
        ...
        if (!empty($_SESSION['member_jwt'])) {
            ...
            // ADMINSESSID üzerinde admin oturumu varsa regenerate admin oturumunu düşürür.
            $canRegenerate = !(function_exists('app_session_has_admin_user') && app_session_has_admin_user())
                && session_name() !== (function_exists('app_admin_session_name') ? app_admin_session_name() : 'ADMINSESSID');
            if ($canRegenerate) {
                try {
    ...
    private static function maybeSyncSessionFromAuthCheck(string $route, string $method, array $result): void
    {
        ...
        if ($token !== '') {
            $_SESSION['member_jwt'] = $token;
            if (function_exists('frontend_set_member_restore_cookie')) {
                frontend_set_member_restore_cookie($token);
            }
        }
    ...
    private static function maybeClearAuthSession(string $route, string $method, array $result): void
    {
        ...
        require_once shared_package_root() . '/config/frontend_session.php';
        if (is_readable(shared_package_root() . '/config/member_api_public.php')) {
            require_once shared_package_root() . '/config/member_api_public.php';
        }
        frontend_session_start();
        if (function_exists('frontend_clear_member_session')) {
            frontend_clear_member_session();
        } elseif (function_exists('app_clear_member_session_keys')) {
    ...
    private static function emitCachedPublicResponse(array $cached): never
    {
        if (!headers_sent()) {
            ...
            header('X-App-Cache: ' . (string) ($cached['cache_state'] ?? 'hit'));
            header('Cache-Control: ' . self::publicCacheControlHeader());
        }
