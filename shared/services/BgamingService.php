...
final class BgamingService
{
    ...
    {
        // Avoid unconditional MODIFY (DDL) — it implicitly commits any open TX.
        foreach ([
            'bgaming_config' => 'currency',
            'bgaming_game_sessions' => 'currency',
        ] as $table => $column) {
            try {
                $stmt = $pdo->prepare(
                ...
                if ($len !== false && (int) $len >= 8) {
                    continue;
                }
                $pdo->exec("ALTER TABLE `{$table}` MODIFY `{$column}` VARCHAR(8) NOT NULL DEFAULT 'USD'");
            } catch (Throwable) {
            }
        }
    }
    ...
    public static function launch(PDO $pdo, ?array $user, array $input): array
    {
        ...
        // Prefer visitor IP from the play shell — game-launch often hits the admin
        // API via an internal proxy that only sees 127.0.0.1.
        $clientIp = trim((string) ($input['ip'] ?? ''));
        if ($clientIp === '' || filter_var($clientIp, FILTER_VALIDATE_IP) === false) {
            $clientIp = self::clientIp();
        }

        // Mirror GSC/aggregator: prefer runtime frontend origin from /play so
        // in-game lobby / deposit links stay on the live host, not a stale config.
        $homeUrl = rtrim(trim((string) ($input['home_url'] ?? '')), '/');
        ...
        if ($returnUrl === '') {
            $returnUrl = self::siteEndpoint();
        }
        $returnUrl = rtrim($returnUrl, '/');
        $depositUrl = trim((string) ($input['deposit_url'] ?? ''));
        if ($depositUrl === '') {
            $depositBase = $homeUrl !== '' ? $homeUrl : rtrim(self::frontendEndpoint(), '/');
            $depositUrl = rtrim($depositBase, '/') . '/deposit';
        }
        ...
        // Upgrade scheme-relative / http launch URLs before the play iframe loads
        // them (mixed content is blocked silently on https:// frontends).
        if (str_starts_with($gameUrl, '//')) {
            $gameUrl = 'https:' . $gameUrl;
        } elseif (preg_match('#^http://#i', $gameUrl) === 1) {
            $gameUrl = 'https://' . substr($gameUrl, 7);
        }
        ...
        return [
            ...
            'data' => [
                ...
                'mode' => $mode,
                'open_mode' => 'iframe',
                'home_url' => $homeUrl !== '' ? $homeUrl : self::frontendEndpoint(),
                'return_url' => $returnUrl,
            ],
        ];
    }
    ...
    private static function applyActions(PDO $pdo, array $payload, array $actions, ?string $forcedType): array
    {
        ...
        $transactions = [];
        // Resolve wallet + warm schema BEFORE beginTransaction. Loyalty/Wagering
        // ensureStorage used to run CREATE/ALTER inside the TX which MySQL
        // implicitly commits — then $pdo->commit() threw "There is no active
        // transaction" and SoftSwiss surfaced client error 204.
        WageringService::ensureSchema($pdo);
        ...
        try {
            ...
            foreach ($actions as $action) {
                ...
                // Defer wagering/loyalty side-effects until after commit — those
                // helpers may run schema DDL (implicit commit) or nested TX.
                if (($type === 'bet' || $type === 'promo_bet') && !$tombstoned && $amount > 0) {
                    $wageringEvents[] = ['op' => 'register', 'amount' => $amount];
                } elseif ($type === 'rollback' && $amount > 0 && in_array($rollbackOriginalType, ['bet', 'promo_bet'], true)) {
                    $wageringEvents[] = ['op' => 'reverse', 'amount' => $amount];
                }
        ...
        foreach ($wageringEvents as $event) {
            try {
                if (($event['op'] ?? '') === 'register') {
                    WageringService::registerBet($pdo, $userId, (float) ($event['amount'] ?? 0), $walletColumn);
                } elseif (($event['op'] ?? '') === 'reverse') {
                    WageringService::reverseBet($pdo, $userId, (float) ($event['amount'] ?? 0), $walletColumn);
                }
            } catch (Throwable $e) {
                error_log('[BgamingService] deferred wagering failed: ' . $e->getMessage());
            }
        }
    ...
    private static function frontendEndpoint(): string
    {
        ...
        $scheme = function_exists('public_url_scheme')
        ...
        if ($fallback === '') {
            if (!function_exists('deploy_domain')) {
                $path = shared_package_root() . '/config/deploy_domains.php';
                if (is_file($path)) {
                    require_once $path;
                }
            }
